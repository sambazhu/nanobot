...
class GatewayHTTPHandler:
    ...
    def _sessions_list_payload(self) -> dict[str, Any]:
        ...
        for s in sessions:
            ...
            if not (isinstance(key, str) and _is_websocket_channel_session_key(key)):
                continue
            row = {
                k: v
                for k, v in s.items()
...
_WEBUI_VISIBLE_SESSION_PREFIXES: tuple[str, ...] = ("weixin:",)


def _is_websocket_channel_session_key(key: str) -> bool:
    return is_webui_session_key(key) or key.startswith(_WEBUI_VISIBLE_SESSION_PREFIXES)
